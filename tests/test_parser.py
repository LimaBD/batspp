...
class TestParser(TestWrapper):
    ...
    def test_is_setup_next(self):
        ...
        parser.tokens = [Token(TokenType.TEXT, 'some text'),
                         Token(TokenType.TEXT, 'some text'),
                         Token(TokenType.EOF, None)]
    ...
    def test_build_test(self):
        ...
        parser.tokens = [Token(TokenType.TEST, '# Test '),
    ...
    def test_build_setup(self):
        """Test for build_setup()"""
        debug.trace(7, f'TestParser.test_build_setup({self})')
        parser = Parser()
    ...
    def test_parse(self):
        ...
        # Check setup
        self.assertEqual(tree.setup.commands, ['some global command', 'another global command'])
        self.assertNotEqual(tree.setup.commands, ['wrong command!'])
